...
export class UploadService {
  ...
  constructor(private readonly configService: ConfigService) {
    this.region = this.configService.get(ENV_KEY.AWS_S3_REGION);
    this.bucketName = this.configService.get(ENV_KEY.AWS_S3_BUCKET_NAME);
    this.accessKeyId = this.configService.get(ENV_KEY.AWS_S3_ACCESS_KEY_ID);
    this.secretAccessKey = this.configService.get(
      ENV_KEY.AWS_S3_SECRET_ACCESS_KEY,
    );
    this.s3URL = this.configService.get(ENV_KEY.AWS_S3_URL);
  }
